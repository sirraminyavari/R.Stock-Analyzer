...
namespace NetTrader.Indicator
{
    ...
    public class TRIX : IndicatorCalculatorBase<SingleDoubleSerie>
    {
        ...
        public TRIX(List<Ohlc> ohlcList, int period)
        {
            this.Load(ohlcList);
            this.Period = period;
        }
        ...
        public override SingleDoubleSerie Calculate()
        {
            ...
            {
                ROC roc = new ROC(OhlcList.Skip(3 * (Period - 1)).ToList(), 1);
                trixSerie = roc.Calculate();
            }
            else
            {
                Momentum momentum = new Momentum(OhlcList.Skip(3 * (Period - 1)).ToList());
